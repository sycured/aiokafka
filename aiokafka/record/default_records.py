...
class _DefaultRecordBatchPy(DefaultRecordBase):
    ...
    def _read_msg(
        ...
        if pos - start_pos != length:
            raise CorruptRecordException(
                f"Invalid record size: expected to read {length} bytes in "
                f"record payload, but instead read {pos - start_pos}")
        self._pos = pos
    ...
    def __next__(self):
        if self._next_record_index >= self._num_records:
            if self._pos != len(self._buffer):
                raise CorruptRecordException(
                    f"{len(self._buffer) - self._pos} unconsumed bytes after "
                    f"all records consumed")
            raise StopIteration
        try:
            msg = self._read_msg()
...
class _DefaultRecordPy:
    ...
    def __repr__(self):
        return (
            f"DefaultRecord(offset={self._offset!r}, "
