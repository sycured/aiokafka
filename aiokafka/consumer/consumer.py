...
pt_must_TP_inst = 'partitions must be TopicPartition instances'
pt_not_assigned = 'Partition is not assigned'
no_pt_cur_ass = 'No partitions are currently assigned'
...
class AIOKafkaConsumer(object):
    ...
    def highwater(self, partition):
        ...
        """
        assert self._subscription.is_assigned(partition), pt_not_assigned
        assignment = self._subscription.subscription.assignment
    ...
    async def seek_to_beginning(self, *partitions):
        ...
        if not all(isinstance(p, TopicPartition) for p in partitions):
            raise TypeError(pt_must_TP_inst)

        if not partitions:
            partitions = self._subscription.assigned_partitions()
            assert partitions, no_pt_cur_ass
        else:
            not_assigned = (
    ...
    async def offsets_for_times(self, timestamps):
        ...
        if self._client.api_version <= (0, 10, 0):
            raise UnsupportedVersionError(
                f"offsets_for_times API not supported for cluster "
                f"version {self._client.api_version}")
        for tp, ts in timestamps.items():
            timestamps[tp] = int(ts)
            if ts < 0:
                raise ValueError(
                    f"The target time for partition {tp} is {ts}. "
                    f"The target time cannot be negative.")
